...
export default function Conviction() {
  return (
    <section id="conviction" className="relative py-28 md:py-44">
      ...
      <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[60vh] w-[60vh] rounded-full"
      ...
      <div className="relative z-10 mx-auto max-w-site px-5 sm:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {beliefs.map((b, i) => (
            ...
            >
              <TiltCard>
                <div className="glass rounded-2xl p-7 sm:p-9 group hover:border-accent/25 transition-colors duration-400 relative overflow-hidden cursor-default card-premium">
                  {/* Number watermark */}
                  <div
                    className="pointer-events-none absolute -right-3 -top-4 font-display text-[110px] leading-none select-none transition-opacity duration-500 group-hover:opacity-[0.06]"
                  ...
                  <span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-accent/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                  {/* Bottom accent line */}
                  <span className="pointer-events-none absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-accent/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

                  {/* Inner glow on hover */}
                  <div
                    className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                    style={{ background: 'radial-gradient(ellipse 60% 40% at 50% 0%, rgba(37,99,235,0.06) 0%, transparent 70%)' }}
                  />

                  <div className="flex items-center justify-between mb-7 relative z-10">
                    <span className="chip">{b.accent}</span>
                  </div>
                  ...
                  <p className="text-[14px] sm:text-[15px] text-ink-2 leading-[1.75] relative z-10" style={{ fontWeight: 300 }}>
                    {b.detail}
                  </p>
                </div>
              </TiltCard>
